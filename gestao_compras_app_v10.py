...
try:
    from sharepoint_auth_v4 import SharePointOAuthClient, SharePointMSALClient
except ImportError:
    try:
...
# --- TELA PRINCIPAL (APÓS LOGIN) ---
    ...
    # PAGE 4: DETALHES DE INTEGRAÇÃO DO SHAREPOINT E OAUTH2
        ...
        if st.session_state.sp_auth_type == "MSAL (Device Flow)":
            ...
            with col_in2:
                st.markdown("<div style='background-color:#F4F6F9; padding: 12px; border-radius: 4px; font-size:12px; border-left: 3px solid #00205B;'><strong>Dica de Desenvolvimento (Sem registrar nada):</strong><br>Se a sua empresa ainda não registrou um app no Azure AD, você pode testar este fluxo usando o Client ID padrão do <strong>SharePoint Online Management Shell</strong> (pré-aprovado pela Microsoft na maioria dos tenants):<br><code style='user-select: all; background-color: #E2E8F0; padding: 2px 4px;'>9bc3ab49-b65d-410a-85ad-de819febfddc</code></div>", unsafe_allow_html=True)
