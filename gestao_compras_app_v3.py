...
st.set_page_config(
    page_title="PGI - Gestão de Cotações v3",
    page_icon="💼",
    layout="wide",
...
st.markdown("""
    ...
    /* Cabeçalho Premium - Logo Integrada e Linha de Destaque Laranja */
    .title-container {
        padding: 24px;
    ...
    /* Cards de Métricas Customizados (Substituição do default do Streamlit) */
    .metric-card-custom {
...
def get_logo_svg(theme="dark"):
    ...
    return f"""
    <svg width="220" height="45" viewBox="0 0 220 45" xmlns="http://www.w3.org/2000/svg" style="vertical-align: middle;">
        <!-- Quadrado Laranja Institucional (Pantone 1505 C) -->
        <rect x="2" y="2" width="41" height="41" rx="4" fill="#FF6F00" />
        <!-- Círculo Azul Institucional (Pantone 2768 C) -->
        <circle cx="22.5" cy="22.5" r="16.5" fill="#00205B" />
        <!-- Símbolo AY em Branco (Geometria do logo) -->
        <path d="M 16,29 L 21.5,14 L 23.5,14 L 29,29" fill="none" stroke="#FFFFFF" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"/>
        <line x1="18.5" y1="23.5" x2="26.5" y2="23.5" stroke="#FFFFFF" stroke-width="2.5" />
        <path d="M 25.5,23.5 L 29,31.5" fill="none" stroke="#FFFFFF" stroke-width="2.5" stroke-linecap="round" />
        <!-- Tipografia Oficial A.YOSHII -->
        <text x="52" y="32" font-family="'Helvetica Neue', Helvetica, Arial, sans-serif" font-size="23" font-weight="900" fill="{text_color}" letter-spacing="1">A.YOSHII</text>
    </svg>
    """
...
if not st.session_state.logged_in:
    logo_light = get_logo_svg(theme="light")
    st.markdown(f"""
        <div style="text-align: center; margin-top: 40px; margin-bottom: 25px;">
            {logo_light}
            <div style="font-size: 13px; color: #8C8C8C; font-weight: 600; text-transform: uppercase; letter-spacing: 2px; margin-top: 10px;">
    ...
    with col2:
        with st.form("login_form"):
            st.markdown("""
                <h3 style="text-align: center; color: #00205B; font-weight: 800; margin-bottom: 15px;">
        ...
        st.markdown("""
            <div style="background-color: #F4F6F9; border-top: 3px solid #FF6F00; padding: 12px; border-radius: 4px; margin-top: 15px; text-align: center;">
                <p style="margin: 0; font-size: 11px; color: #1E1E1E;">
                    💡 <strong>Credenciais de Demonstração:</strong><br>
                    Usuário: <code style="background-color: #E2E8F0; padding: 2px 4px; border-radius: 2px;">admin</code> | 
                    Senha: <code style="background-color: #E2E8F0; padding: 2px 4px; border-radius: 2px;">1234</code>
                </p>
            </div>
        """, unsafe_allowed_html=True)
...
# --- TELA PRINCIPAL (APÓS LOGIN) ---
    ...
    with st.sidebar:
        logo_dark = get_logo_svg(theme="dark")
        st.markdown(f"""
            <div style="padding: 10px 0; border-bottom: 1px solid rgba(255,255,255,0.15); margin-bottom: 15px; text-align: center;">
                {logo_dark}
            </div>
        """, unsafe_allowed_html=True)
        
        st.markdown(f"👤 <strong>Comprador Ativo:</strong><br><span style='color: #FF6F00; font-weight: bold;'>{st.session_state.user}</span>", unsafe_allowed_html=True)
        ...
        if st.session_state.sp_connected:
            ...
            st.markdown("""
                <div style="background-color: rgba(255,111,0,0.15); border: 1px solid #FF6F00; padding: 8px; border-radius: 4px; font-size: 12px; margin-bottom: 10px;">
    ...
    # Título do Painel Conectado (Identidade A.Yoshii Premium)
    logo_header = get_logo_svg(theme="dark")
    st.markdown(f"""
    ...
    if menu_option == "Dashboard Geral":
        st.markdown(f"<h3 class='styled-table-title'>📊 Indicadores de Performance ({st.session_state.db_mode})</h3>", unsafe_allowed_html=True)
        ...
        # Exibição de Métricas Principais usando Cards Customizados (Manual de Identidade Visual)
        ...
        st.markdown("<h3 class='styled-table-title'>📋 Lista Consolidada de Processos de Cotação</h3>", unsafe_allowed_html=True)
    ...
    # PAGE 2: LANÇAR NOVA COTAÇÃO
        ...
        st.markdown(f"<h3 class='styled-table-title'>🆕 Cadastrar Novo Processo Interno ({st.session_state.db_mode})</h3>", unsafe_allowed_html=True)
        ...
        with st.form("new_record_form"):
            ...
            with col_f1:
                st.markdown("<strong style='color:#00205B;'>Dados Gerais da Cotação</strong>", unsafe_allowed_html=True)
            ...
            with col_f2:
                st.markdown("<strong style='color:#00205B;'>Etapas de Validação Inicial</strong>", unsafe_allowed_html=True)
            ...
            st.markdown("<hr style='border-color:#F4F6F9;'>", unsafe_allowed_html=True)
            col_f3, col_f4 = st.columns(2)
            with col_f3:
                st.markdown("<strong style='color:#00205B;'>Etapas de Contratação & Sistemas</strong>", unsafe_allowed_html=True)
            ...
            with col_f4:
                st.markdown("<strong style='color:#00205B;'>Assinatura & Auditoria de Pasta</strong>", unsafe_allowed_html=True)
    ...
    # PAGE 3: GERENCIAMENTO E EDIÇÃO DE REGISTROS
        ...
        st.markdown("<h3 class='styled-table-title'>✏️ Atualizar Status e Fluxos das Cotações</h3>", unsafe_allowed_html=True)
        ...
        if df_current.empty:
            ...
            if selected_option:
                ...
                with st.form("edit_record_form"):
                    st.markdown(f"""
                        <div style="background-color: #F4F6F9; padding: 10px; border-radius: 4px; border-left: 4px solid #FF6F00; margin-bottom: 20px;">
                    ...
                    with col_e1:
                        st.markdown("<strong style='color:#00205B;'>Dados do Processo</strong>", unsafe_allowed_html=True)
                        ...
                        st.markdown("<br><strong style='color:#00205B;'>Status de Compliance e Validação</strong>", unsafe_allowed_html=True)
                    ...
                    with col_e2:
                        st.markdown("<strong style='color:#00205B;'>Aprovações Suprimentos</strong>", unsafe_allowed_html=True)
                        ...
                        st.markdown("<br><strong style='color:#00205B;'>Contratação & Minuta</strong>", unsafe_allowed_html=True)
    ...
    # PAGE 4: DETALHES DE INTEGRAÇÃO DO SHAREPOINT E OAUTH2
        ...
        st.markdown("<h3 class='styled-table-title'>🔑 Painel de Integração Ativa via OAuth2 (Microsoft Entra ID)</h3>", unsafe_allowed_html=True)
        
        with st.form("oauth2_config_form"):
            st.markdown("<strong style='color:#00205B;'>Configurações de Identidade Microsoft Cloud</strong>", unsafe_allowed_html=True)
        ...
        st.markdown("<br>", unsafe_allowed_html=True)
        st.markdown("<h3 class='styled-table-title'>📐 Arquitetura de Integração e Ecossistema</h3>", unsafe_allowed_html=True)
        ...
        st.markdown("""
        O aplicativo v3 está pareado com o arquivo de suporte <code>sharepoint_auth.py</code> para fazer a comunicação direta sem dependências complexas externas. Ele utiliza o fluxo seguro de <strong>Client Credentials</strong> do OAuth2.
