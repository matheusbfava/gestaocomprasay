...
# Adiciona o diretório atual ao path para garantir que possamos importar o módulo de autenticação v2
...
st.set_page_config(
    page_title="PGI - Gestão de Cotações v6",
    page_icon="💼",
    layout="wide",
...
st.markdown("""
    ...
    /* Alinhamento de colunas de tabela interativa */
    .action-btn-col {
        display: flex;
        gap: 5px;
    }
...
# --- FUNÇÃO PARA CARREGAR STATUS BADGE ---
def render_status_badge(status):
    status_lower = str(status).strip().lower()
    if status_lower == "ok":
        return '<span style="background-color: #EAF7EE; color: #28A745; padding: 2px 8px; border-radius: 4px; font-weight: bold; font-size: 11px; display: inline-block;">OK</span>'
    elif status_lower == "n/a":
        return '<span style="background-color: #F4F6F9; color: #8C8C8C; padding: 2px 8px; border-radius: 4px; font-weight: bold; font-size: 11px; display: inline-block;">N/A</span>'
    else:
        return '<span style="background-color: #FFF3CD; color: #FF6F00; padding: 2px 8px; border-radius: 4px; font-weight: bold; font-size: 11px; display: inline-block;">aguardando</span>'
    ...
    st.session_state.db_data = [
        {
            ...
            "orcamento": "OK",
            "validacao_eng": "OK",
            "validacao_ger": "OK",            "validacao_sup": "OK",
            "req_mega": "OK",
            "contr_mega": "OK",
            "param_fiscal": "OK",            "minuta": "OK",
            "ass_digital": "OK",
            "credenciamento": "OK",            "comunicar": "OK",
            "savings": "1050.00",
            "aud_pasta": "OK",
        ...
        {            "ID_PGI": "46252",
            "cotacao": "GAIA ELÉTRICA ALLEGRO",            "due_dilligence": "OK",
            "equalizacao": "OK",
            "orcamento": "N/A",
            "validacao_eng": "OK",
            "validacao_ger": "OK",
            "validacao_sup": "OK",            "req_mega": "OK",            "contr_mega": "OK",            "param_fiscal": "OK",            "minuta": "OK",            "ass_digital": "aguardando",            "credenciamento": "OK",            "comunicar": "OK",            "savings": "75000.00",            "aud_pasta": "aguardando",            "valor_fechado": "980000.00"        },
        {            "ID_PGI": "43405",            "cotacao": "PAISAGISMO DUETTO",            "due_dilligence": "OK",            "equalizacao": "OK",            "orcamento": "N/A",            "validacao_eng": "OK",            "validacao_ger": "OK",            "validacao_sup": "OK",            "req_mega": "OK",            "contr_mega": "OK",            "param_fiscal": "OK",            "minuta": "OK",            "ass_digital": "aguardando",            "credenciamento": "OK",            "comunicar": "N/A",            "savings": "52406.72",            "aud_pasta": "aguardando",            "valor_fechado": "145073.39"        },
        {            "ID_PGI": "47085",            "cotacao": "APLICAÇÃO DE FUNDO LIV",            "due_dilligence": "OK",            "equalizacao": "OK",            "orcamento": "N/A",            "validacao_eng": "OK",            "validacao_ger": "OK",            "validacao_sup": "OK",            "req_mega": "OK",            "contr_mega": "OK",            "param_fiscal": "N/A",            "minuta": "OK",            "ass_digital": "aguardando",            "credenciamento": "N/A",            "comunicar": "N/A",            "savings": "0.00",            "aud_pasta": "aguardando",            "valor_fechado": "16320.00"        }    ]
...
# --- CARREGAMENTO DOS GRUPOS DE INSUMO ---
def carregar_grupos_insumo():
    """
    Busca os nomes dos grupos de insumo direto do SharePoint (dSUPRI_GruposInsumo / NomeGrupo)
    se estiver conectado em modo Live. Caso contrário, retorna uma lista padrão de fallback.
    """
    if st.session_state.db_mode == "SharePoint (Live)" and sp_client:
        try:
            # Inicializa um cliente secundário para apontar ao site de Projetos
            ...
            # Ordena e remove duplicidades
            grupos = sorted(list(set(grupos)))
            if grupos:
                return grupos
        except Exception as e:
            st.sidebar.warning(f"⚠️ Não foi possível carregar os grupos do SharePoint: {str(e)}")
            
    # Lista de Fallback/Simulada baseada no ecossistema e nos dados reais
    return [
        "INOX E VIDRO HORIZON",
        "ADITIVO FUNDAÇÃO ALTANA - HCM",
        "ADITIVO TERRAPLANAGEM GAIA",
        "GAIA ELÉTRICA ALLEGRO",
        "PAISAGISMO DUETTO",
        "APLICAÇÃO DE FUNDO LIV",
        ...
        try:
            ...
            for item in items:
                dados_mapeados.append({
                    ...
                    "sp_id": item.get("ID"),  # Armazena o ID primário interno do SharePoint para exclusão e merge
...
def excluir_registro(pgi_id):
    if st.session_state.db_mode == "SharePoint (Live)" and sp_client:
        try:
            # Pede os dados em formato bruto para identificar o ID de linha do SharePoint (PrimaryKey)
            items = sp_client.get_list_items()
        ...
        # Modo simulado: remove da lista da sessão
...
# --- TELA PRINCIPAL (APÓS LOGIN) ---
    ...
    lista_grupos = carregar_grupos_insumo()
    ...
    with st.sidebar:
        ...
        # Sincroniza navegação baseada no estado
        options_nav = ["Dashboard Geral", "Lançar Nova Cotação", "Gerenciamento de Registros", "Integração SharePoint"]
        menu_option = st.radio(
            "Navegação",
            options_nav,
            index=options_nav.index(st.session_state.menu_option)
        )
    ...
    if st.session_state.menu_option == "Dashboard Geral":
        ...
        if st.session_state.confirm_delete_id:
            ...
            item_del = next((item for item in db_data_current if str(item["ID_PGI"]) == str(del_id)), None)
            if item_del:
                st.warning(f"⚠️ **CONFIRMAÇÃO DE EXCLUSÃO:** Tem certeza que deseja apagar o registro **ID PGI {del_id} - {item_del['cotacao']}** permanentemente do banco de dados?")
                col_c1, col_c2, col_c3 = st.columns([1.5, 1.5, 7])
                with col_c1:
                    if st.button("✔️ Sim, Excluir", key="confirm_yes_btn"):
                        if excluir_registro(del_id):
                            st.session_state.confirm_delete_id = None
        ...
        # Faz os cálculos matemáticos dos indicadores baseados em dados reais
        df_calc_full = df_current.copy()
        if not df_calc_full.empty:
            df_calc_full["valor_fechado_num"] = df_calc_full["valor_fechado"].astype(float)
            df_calc_full["savings_num"] = df_calc_full["savings"].astype(float)
            
            total_fechado_all = df_calc_full["valor_fechado_num"].sum()
            total_savings_all = df_calc_full["savings_num"].sum()
            ...
            total_fechado_all = total_savings_all = taxa_economia_all = 0
        ...
        with col_m1:
            render_html(f"""
                <div class="metric-card-custom">
                    ...
                    <div class="metric-value">{len(df_calc_full)}</div>
                </div>
            """)
        with col_m2:
        ...
        with st.expander("🔍 Filtros de Pesquisa por Coluna (Modo Contém / Pesquisa Parcial)", expanded=False):
            ...
            with col_f7:
                f_req = st.selectbox("Req. Mega", get_filter_options(df_current, "req_mega"), key="f_req_sel")
            with col_f8:
                f_contr = st.selectbox("Contrato Mega", get_filter_options(df_current, "contr_mega"), key="f_contr_sel")

            col_f9, col_f10, col_f11, col_f12 = st.columns(4)
            with col_f9:
                f_minuta = st.selectbox("Minuta", get_filter_options(df_current, "minuta"), key="f_minuta_sel")
            with col_f10:
                f_ass = st.selectbox("Ass. Digital", get_filter_options(df_current, "ass_digital"), key="f_ass_sel")
            with col_f11:
                f_aud = st.selectbox("Audit. Pasta", get_filter_options(df_current, "aud_pasta"), key="f_aud_sel")
            with col_f12:
                # Botão rápido para resetar filtros
        ...
        if not df_filtered.empty:
            ...
            if f_req != "Todos":
                df_filtered = df_filtered[df_filtered["req_mega"].astype(str).str.contains(f_req, case=False, na=False)]
            if f_contr != "Todos":
                df_filtered = df_filtered[df_filtered["contr_mega"].astype(str).str.contains(f_contr, case=False, na=False)]
            if f_minuta != "Todos":
                df_filtered = df_filtered[df_filtered["minuta"].astype(str).str.contains(f_minuta, case=False, na=False)]
            if f_ass != "Todos":
                df_filtered = df_filtered[df_filtered["ass_digital"].astype(str).str.contains(f_ass, case=False, na=False)]
            if f_aud != "Todos":
        ...
        if not df_filtered.empty:
            ...
            # Cabeçalho da Tabela
            col_h1, col_h2, col_h3, col_h4, col_h5, col_h6, col_h7, col_h8 = st.columns([1.2, 4.2, 1.4, 1.4, 1.4, 1.6, 1.4, 1.4])
            with col_h1: st.markdown("<strong style='color:#00205B;'>ID PGI</strong>", unsafe_allow_html=True)
            with col_h2: st.markdown("<strong style='color:#00205B;'>Escopo de Cotação</strong>", unsafe_allow_html=True)
            with col_h3: st.markdown("<strong style='color:#00205B;'>Solic. Orç.</strong>", unsafe_allow_html=True)
            with col_h4: st.markdown("<strong style='color:#00205B;'>Due Dill.</strong>", unsafe_allow_html=True)
            with col_h5: st.markdown("<strong style='color:#00205B;'>Equaliz.</strong>", unsafe_allow_html=True)
            with col_h6: st.markdown("<strong style='color:#00205B;'>Savings (R$)</strong>", unsafe_allow_html=True)
            with col_h7: st.markdown("<strong style='color:#00205B;'>Aud. Pasta</strong>", unsafe_allow_html=True)
            with col_h8: st.markdown("<strong style='color:#00205B; text-align: center; display: block;'>Ações</strong>", unsafe_allow_html=True)
            st.markdown("<hr style='margin: 4px 0; border-color:#FF6F00;'>", unsafe_allow_html=True)
            
            # Linhas da tabela
            for index, row in df_filtered.iterrows():
                col_r1, col_r2, col_r3, col_r4, col_r5, col_r6, col_r7, col_r8 = st.columns([1.2, 4.2, 1.4, 1.4, 1.4, 1.6, 1.4, 1.4])
    ...
    # PAGE 2: LANÇAR NOVA COTAÇÃO
        ...
        with st.form("new_record_form"):
            ...
            with col_f1:
                ...
                # ID_PGI aceitando somente números inteiros como solicitado
                new_id = st.number_input("ID_PGI (Apenas números inteiros)", min_value=1000, max_value=999999, step=1, value=49000, format="%d")
                new_cotacao = st.selectbox("Escopo da Cotação", lista_grupos)
                ...
                # Solicitante / Orçamento alterado para 'Solicitar Orçamento' com Dropdown OK, N/A, aguardando
            ...
            if submit_new:
                if str(new_id) in df_current["ID_PGI"].values:
...
# PAGE 3: GERENCIAMENTO E EDIÇÃO DE REGISTROS
        ...
        if df_current.empty:
            ...
            # Identifica se há um PGI pré-selecionado pelo ícone de lápis do Dashboard
            default_index_edit = 0
            if st.session_state.selected_pgi_to_edit:
                search_id = str(st.session_state.selected_pgi_to_edit)
                for idx, opt in enumerate(list_ids):
                    if opt.startswith(f"{search_id} -"):
                        default_index_edit = idx
                        break
                # Consome o estado para as próximas edições
                st.session_state.selected_pgi_to_edit = None
            
            selected_option = st.selectbox("Selecione o Processo Interno para Editar", list_ids, index=default_index_edit)
            ...
            if selected_option:
                ...
                with st.form("edit_record_form"):
                    ...
                    with col_e1:
                        ...
                        # Escopo como selectbox mapeado para os grupos
                        default_idx_escopo = 0
                        if item["cotacao"] in lista_grupos:
                            default_idx_escopo = lista_grupos.index(item["cotacao"])
                        edit_cotacao = st.selectbox("Escopo de Cotação", lista_grupos, index=default_idx_escopo)
                        ...
                        # Orçamento alterado para seletor de status
                    ...
                    if submit_edit:
                        ...
                        if st.session_state.db_mode == "SharePoint (Live)" and sp_client:
                            try:
                                # Tenta usar o ID primário mapeado
                                sp_id = item.get("sp_id")
                                if not sp_id:
                                    # Fallback tentando achar por PGI ID
                                    items_bruto = sp_client.get_list_items()
                                    target_bruto = next((ib for ib in items_bruto if str(ib.get("ID_PGI", "")) == str(selected_id)), None)
                                    if target_bruto:
                                        sp_id = target_bruto.get("ID")
                                
                                if sp_id:
                                    sp_client.update_list_item(int(sp_id), updated_fields)
                                    st.success("✔️ Registro atualizado com sucesso DIRETAMENTE no SharePoint!")
                                else:
                                    raise Exception("ID de registro do SharePoint não localizado.")
                            except Exception as e:
...
# PAGE 4: DETALHES DE INTEGRAÇÃO DO SHAREPOINT E OAUTH2
        ...
        st.markdown("""
        O aplicativo v6 está pareado com o arquivo de suporte <code>sharepoint_auth_v2.py</code> para fazer a comunicação direta sem dependências complexas externas. Ele utiliza o fluxo seguro de <strong>Client Credentials</strong> do OAuth2.
