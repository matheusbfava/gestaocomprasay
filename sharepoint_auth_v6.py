import requests
import json
import msal

class SharePointOAuthClient:
    """
    Cliente de Autenticação OAuth2 e Integração para SharePoint Online (v2).
    Utiliza o fluxo 'Client Credentials Grant' do Microsoft Entra ID (Azure AD)
    para autenticação de aplicativo (App-Only) com Client ID e Client Secret.
    """
    
    def __init__(self, tenant_id: str, client_id: str, client_secret: str, site_url: str = "https://grupoayoshii.sharepoint.com/sites/DPTO_SUPRIMENTOS"):
        self.tenant_id = tenant_id
        self.client_id = client_id
        self.client_secret = client_secret
        self.site_url = site_url.rstrip('/')
        self.access_token = None
        self.expires_in = 0
        
        # O escopo padrão para APIs do SharePoint Online é baseado na URL raiz do tenant
        base_tenant_url = site_url.split('/sites/')[0]
        self.scope = f"{base_tenant_url}/.default"
        
    def acquire_token(self) -> str:
        """
        Obtém o Token de Acesso OAuth2 junto ao Microsoft Entra ID.
        """
        if not self.tenant_id or not self.client_id or not self.client_secret:
            raise ValueError("As credenciais (Tenant ID, Client ID, Client Secret) são obrigatórias.")
            
        token_url = f"https://login.microsoftonline.com/{self.tenant_id}/oauth2/v2.0/token"
        
        headers = {
            "Content-Type": "application/x-www-form-urlencoded"
        }
        
        payload = {
            "grant_type": "client_credentials",
            "client_id": self.client_id,
            "client_secret": self.client_secret,
            "scope": self.scope
        }
        
        try:
            response = requests.post(token_url, headers=headers, data=payload, timeout=10)
            
            if response.status_code == 200:
                token_data = response.json()
                self.access_token = token_data.get("access_token")
                self.expires_in = token_data.get("expires_in", 3600)
                return self.access_token
            else:
                error_description = response.json().get("error_description", response.text)
                raise Exception(f"Falha na autenticação OAuth2 (HTTP {response.status_code}): {error_description}")
                
        except requests.exceptions.RequestException as e:
            raise ConnectionError(f"Erro ao conectar com o servidor do Microsoft Entra ID: {str(e)}")

    def _get_auth_headers(self) -> dict:
        """
        Gera os cabeçalhos padrão de autenticação do SharePoint REST API.
        """
        if not self.access_token:
            self.acquire_token()
            
        return {
            "Authorization": f"Bearer {self.access_token}",
            "Accept": "application/json;odata=verbose",
            "Content-Type": "application/json;odata=verbose"
        }

    def get_list_items(self, list_name: str = "PGI_GestaoCotacoes") -> list:
        """
        Busca todos os registros da lista especificada no SharePoint.
        """
        endpoint = f"{self.site_url}/_api/web/lists/getbytitle('{list_name}')/items"
        headers = self._get_auth_headers()
        
        try:
            response = requests.get(endpoint, headers=headers, timeout=15)
            
            if response.status_code == 200:
                data = response.json()
                return data.get("d", {}).get("results", [])
            else:
                raise Exception(f"Erro ao ler lista do SharePoint (HTTP {response.status_code}): {response.text}")
                
        except requests.exceptions.RequestException as e:
            raise ConnectionError(f"Erro de conexão com o SharePoint: {str(e)}")

    def insert_list_item(self, item_data: dict, list_name: str = "PGI_GestaoCotacoes") -> dict:
        """
        Insere um novo registro (item) na lista do SharePoint.
        """
        endpoint = f"{self.site_url}/_api/web/lists/getbytitle('{list_name}')/items"
        headers = self._get_auth_headers()
        
        list_entity_type = f"SP.Data.{list_name}ListItem"
        
        payload = {
            "__metadata": {
                "type": list_entity_type
            }
        }
        payload.update(item_data)
        
        try:
            response = requests.post(endpoint, headers=headers, data=json.dumps(payload), timeout=15)
            
            if response.status_code == 201:
                return response.json().get("d", {})
            else:
                raise Exception(f"Erro ao inserir item (HTTP {response.status_code}): {response.text}")
                
        except requests.exceptions.RequestException as e:
            raise ConnectionError(f"Erro de conexão ao inserir registro: {str(e)}")

    def update_list_item(self, item_id: int, item_data: dict, list_name: str = "PGI_GestaoCotacoes") -> bool:
        """
        Atualiza um registro existente na lista do SharePoint usando o ID interno da linha (Primary Key).
        """
        endpoint = f"{self.site_url}/_api/web/lists/getbytitle('{list_name}')/items({item_id})"
        
        headers = self._get_auth_headers()
        headers.update({
            "X-HTTP-Method": "MERGE",
            "IF-MATCH": "*"
        })
        
        list_entity_type = f"SP.Data.{list_name}ListItem"
        payload = {
            "__metadata": {
                "type": list_entity_type
            }
        }
        payload.update(item_data)
        
        try:
            response = requests.post(endpoint, headers=headers, data=json.dumps(payload), timeout=15)
            
            if response.status_code in [200, 204]:
                return True
            else:
                raise Exception(f"Erro ao atualizar item (HTTP {response.status_code}): {response.text}")
                
        except requests.exceptions.RequestException as e:
            raise ConnectionError(f"Erro de conexão ao atualizar registro: {str(e)}")

    def delete_list_item(self, item_id: int, list_name: str = "PGI_GestaoCotacoes") -> bool:
        """
        Exclui um registro existente na lista do SharePoint usando o ID interno da linha (Primary Key).
        """
        endpoint = f"{self.site_url}/_api/web/lists/getbytitle('{list_name}')/items({item_id})"
        
        headers = self._get_auth_headers()
        headers.update({
            "X-HTTP-Method": "DELETE",
            "IF-MATCH": "*"
        })
        
        try:
            response = requests.post(endpoint, headers=headers, timeout=15)
            
            if response.status_code in [200, 204]:
                return True
            else:
                raise Exception(f"Erro ao excluir item (HTTP {response.status_code}): {response.text}")
                
        except requests.exceptions.RequestException as e:
            raise ConnectionError(f"Erro de conexão ao excluir registro: {str(e)}")


class SharePointMSALClient:
    """
    Cliente de Autenticação OAuth2 via MSAL (Microsoft Authentication Library) para SharePoint Online.
    Utiliza o fluxo de 'Device Code Flow' de forma delegada para rodar em nome do usuário
    conectado (on-behalf-of-user), eliminando a necessidade de credenciais de Administrador da TI
    ou Client Secret.
    """
    
    def __init__(self, tenant_id: str, client_id: str, site_url: str = "https://grupoayoshii.sharepoint.com/sites/DPTO_SUPRIMENTOS", use_default_scope: bool = True):
        self.tenant_id = tenant_id
        self.client_id = client_id
        self.site_url = site_url.rstrip('/')
        self.authority = f"https://login.microsoftonline.com/{tenant_id}"
        
        # Escopo delegado para acesso a arquivos e listas do SharePoint Online
        base_tenant_url = site_url.split('/sites/')[0]
        
        # Oferecemos o .default como escopo padrão para respeitar estritamente a pré-autorização nativa de cada cliente da Microsoft
        if use_default_scope:
            self.scopes = [f"{base_tenant_url}/.default"]
        else:
            self.scopes = [f"{base_tenant_url}/AllSites.Write", f"{base_tenant_url}/AllSites.Read"]
        
        self.access_token = None
        self.app = msal.PublicClientApplication(
            self.client_id,
            authority=self.authority
        )
        
    def initiate_device_flow(self) -> dict:
        """
        Inicia o fluxo de código de dispositivo (Device Code Flow).
        Retorna o dicionário contendo 'user_code', 'verification_uri' e 'message'.
        """
        if not self.tenant_id or not self.client_id:
            raise ValueError("Tenant ID e Client ID são obrigatórios.")
            
        flow = self.app.initiate_device_flow(scopes=self.scopes)
        if "user_code" not in flow:
            raise Exception(f"Falha ao iniciar o fluxo de código de dispositivo: {json.dumps(flow)}")
        return flow

    def acquire_token_by_device_flow(self, flow: dict) -> str:
        """
        Aguarda a autenticação do usuário no navegador e adquire o token.
        Essa chamada faz polling periódico nos servidores do Azure AD.
        """
        result = self.app.acquire_token_by_device_flow(flow)
        if "access_token" in result:
            self.access_token = result["access_token"]
            return self.access_token
        else:
            error_msg = result.get("error_description", result.get("error", "Autenticação cancelada ou expirada."))
            raise Exception(f"Falha na autenticação via Device Code: {error_msg}")

    def _get_auth_headers(self) -> dict:
        """
        Gera os cabeçalhos padrão de autenticação do SharePoint REST API.
        """
        if not self.access_token:
            raise Exception("Não autenticado. Por favor, realize o login via Device Code primeiro.")
            
        return {
            "Authorization": f"Bearer {self.access_token}",
            "Accept": "application/json;odata=verbose",
            "Content-Type": "application/json;odata=verbose"
        }

    def get_list_items(self, list_name: str = "PGI_GestaoCotacoes") -> list:
        endpoint = f"{self.site_url}/_api/web/lists/getbytitle('{list_name}')/items"
        headers = self._get_auth_headers()
        
        try:
            response = requests.get(endpoint, headers=headers, timeout=15)
            if response.status_code == 200:
                data = response.json()
                return data.get("d", {}).get("results", [])
            else:
                raise Exception(f"Erro ao ler lista do SharePoint (HTTP {response.status_code}): {response.text}")
        except requests.exceptions.RequestException as e:
            raise ConnectionError(f"Erro de conexão com o SharePoint: {str(e)}")

    def insert_list_item(self, item_data: dict, list_name: str = "PGI_GestaoCotacoes") -> dict:
        endpoint = f"{self.site_url}/_api/web/lists/getbytitle('{list_name}')/items"
        headers = self._get_auth_headers()
        list_entity_type = f"SP.Data.{list_name}ListItem"
        
        payload = {
            "__metadata": {
                "type": list_entity_type
            }
        }
        payload.update(item_data)
        
        try:
            response = requests.post(endpoint, headers=headers, data=json.dumps(payload), timeout=15)
            if response.status_code == 201:
                return response.json().get("d", {})
            else:
                raise Exception(f"Erro ao inserir item (HTTP {response.status_code}): {response.text}")
        except requests.exceptions.RequestException as e:
            raise ConnectionError(f"Erro de conexão ao inserir registro: {str(e)}")

    def update_list_item(self, item_id: int, item_data: dict, list_name: str = "PGI_GestaoCotacoes") -> bool:
        endpoint = f"{self.site_url}/_api/web/lists/getbytitle('{list_name}')/items({item_id})"
        headers = self._get_auth_headers()
        headers.update({
            "X-HTTP-Method": "MERGE",
            "IF-MATCH": "*"
        })
        list_entity_type = f"SP.Data.{list_name}ListItem"
        payload = {
            "__metadata": {
                "type": list_entity_type
            }
        }
        payload.update(item_data)
        
        try:
            response = requests.post(endpoint, headers=headers, data=json.dumps(payload), timeout=15)
            if response.status_code in [200, 204]:
                return True
            else:
                raise Exception(f"Erro ao atualizar item (HTTP {response.status_code}): {response.text}")
        except requests.exceptions.RequestException as e:
            raise ConnectionError(f"Erro de conexão ao atualizar registro: {str(e)}")

    def delete_list_item(self, item_id: int, list_name: str = "PGI_GestaoCotacoes") -> bool:
        endpoint = f"{self.site_url}/_api/web/lists/getbytitle('{list_name}')/items({item_id})"
        headers = self._get_auth_headers()
        headers.update({
            "X-HTTP-Method": "DELETE",
            "IF-MATCH": "*"
        })
        
        try:
            response = requests.post(endpoint, headers=headers, timeout=15)
            if response.status_code in [200, 204]:
                return True
            else:
                raise Exception(f"Erro ao excluir item (HTTP {response.status_code}): {response.text}")
        except requests.exceptions.RequestException as e:
            raise ConnectionError(f"Erro de conexão ao excluir registro: {str(e)}")
